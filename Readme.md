<!-- ========== TOP HEADER BANNER ========== -->
<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&section=header&color=0:0f2027,50:203a43,100:2c5364&height=180&fontColor=ffffff&text=Umar%20(UMKA)%20Sharshembiev&fontSize=40&animation=twinkling"/>
</p>

<h2 align="center">🚀 Senior-Minded Frontend Developer • React • JavaScript • UI/UX</h2>

<p align="center">
  <img src="https://readme-typing-svg.herokuapp.com/?color=6dd5ed&size=24&center=true&vCenter=true&width=700&lines=Создаю+интуитивные+и+динамичные+интерфейсы;Разрабатываю+масштабируемые+React+приложения;Делаю+UI+красивым,+гладким+и+анимированным;Стремлюсь+к+Senior+уровню" />
</p>

---

## ✨ О себе

Привет! Я — **Умар (UMKA) Sharshembiev**, фронтенд-разработчик, который не просто пишет код, а создаёт **живые и отзывчивые интерфейсы**, где каждый элемент продуман до деталей:

- ⚡ **Высокопроизводительные веб-приложения** с плавным UX  
- 🎨 **Современные UI/UX решения** с анимациями и интерактивом  
- ⚛️ **React-first подход**: функциональные компоненты, hooks, state management  
- 🧩 **Чистая архитектура и масштабируемый код**  

💻 Технологии: **HTML5, CSS3, JavaScript (ES6+), React**, знаком с **TypeScript, Java и Dart**.

---

## 🛠 Навыки

<p align="center">
  <img src="https://img.shields.io/badge/HTML5-0f2027?style=for-the-badge&logo=html5&logoColor=white"/>
  <img src="https://img.shields.io/badge/CSS3-203a43?style=for-the-badge&logo=css3&logoColor=white"/>
  <img src="https://img.shields.io/badge/JavaScript-2c5364?style=for-the-badge&logo=javascript&logoColor=%23F7DF1E"/>
  <img src="https://img.shields.io/badge/React-0f2027?style=for-the-badge&logo=react&logoColor=61DAFB"/>
  <img src="https://img.shields.io/badge/TypeScript-203a43?style=for-the-badge&logo=typescript&logoColor=white"/>
  <img src="https://img.shields.io/badge/Java-2c5364?style=for-the-badge&logo=java&logoColor=white"/>
  <img src="https://img.shields.io/badge/Dart-0f2027?style=for-the-badge&logo=dart&logoColor=white"/>
  <img src="https://img.shields.io/badge/Git-203a43?style=for-the-badge&logo=git&logoColor=white"/>
</p>

---

## 🌌 Проект ORION

> **Orion** — персональный навигатор школьников Кыргызстана в мире образования и карьеры.  
> Помогает выбрать профессию, план развития и открыть возможности будущего.  
> Реализован с акцентом на удобный UI и масштабируемую архитектуру.

---

## 🎯 Избранные проекты

| Проект | Описание | Ссылка |
|--------|----------|--------|
| 🎬 **Films App** | Онлайн-каталог фильмов с динамическим поиском и сортировкой | [Открыть](https://films-liard.vercel.app/) |
| 💼 **Портфолио сайт** | Личный сайт с современным UI, анимациями и адаптивным дизайном | [Открыть](https://portfolio-three-dusky-9kiuici1pk.vercel.app/) |
| 🎮 **Мини-игра** | Интерактивная игра на чистом JavaScript | [game.html] |

---

## 📈 GitHub Статистика

<p align="center">
  <img height="165" src="https://github-readme-stats.vercel.app/api?username=Roma-ejj&show_icons=true&theme=tokyonight&border_radius=10"/>
  <img height="165" src="https://github-readme-stats.vercel.app/api/top-langs/?username=Roma-ejj&layout=compact&theme=tokyonight&border_radius=10"/>
</p>

---

## 🔥 Позиционирование

<p align="center">
  <img src="https://img.shields.io/badge/Level-Junior%2FMiddle→Senior%20in%20Progress-6dd5ed?style=for-the-badge&logo=rocket&logoColor=black"/>
</p>

- 🚀 Фокус на **React, анимации, продвинутую архитектуру приложений**  
- 🔍 Постоянно оптимизирую код и интерфейсы  
- ⚡ Ищу **вызовы, стажировки и команду для амбициозных проектов**

---

## 📞 Контакты

<p align="center">
  <a href="tel:+996705414210"><img src="https://img.shields.io/badge/Телефон-0f2027?style=for-the-badge&logo=phone&logoColor=white"></a>
  <a href="https://t.me/programm_14"><img src="https://img.shields.io/badge/Telegram-203a43?style=for-the-badge&logo=telegram&logoColor=white"></a>
  <a href="https://instagram.com/programm_14"><img src="https://img.shields.io/badge/Instagram-2c5364?style=for-the-badge&logo=instagram&logoColor=white"></a>
</p>

---

<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&section=footer&color=0:0f2027,100:2c5364&height=140"/>
</p>
h1
g
